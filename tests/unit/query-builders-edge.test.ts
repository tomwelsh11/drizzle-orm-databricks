...
describe("SELECT edge cases", () => {
  it("multiple where() calls — last one wins", () => {
    ...
    const compiled = builder.from(users).where(eq(users.id, "a")).where(eq(users.id, "b")).toSQL();
  ...
  it("where() with ne", () => {
    const compiled = makeSelectBuilder().from(users).where(ne(users.id, "x")).toSQL();
  ...
  it("where() with like", () => {
    const compiled = makeSelectBuilder().from(users).where(like(users.name, "%ali%")).toSQL();
    expect(compiled.sql.toLowerCase()).toContain("like");
    expect(compiled.params).toEqual(["%ali%"]);
  });

  it("where() with isNull", () => {
    const compiled = makeSelectBuilder().from(users).where(isNull(users.name)).toSQL();
    expect(compiled.sql.toLowerCase()).toContain("is null");
    expect(compiled.params).toEqual([]);
  });

  it("where() with isNotNull", () => {
    const compiled = makeSelectBuilder().from(users).where(isNotNull(users.name)).toSQL();
  ...
  it("orderBy() with explicit asc()", () => {
    const compiled = makeSelectBuilder().from(users).orderBy(asc(users.age)).toSQL();
  ...
  it("orderBy() with multiple columns mixing asc and desc", () => {
    const compiled = makeSelectBuilder()
      ...
      .toSQL();
    expect(compiled.sql.toLowerCase()).toContain("order by");
    const orderByPart = compiled.sql
      .toLowerCase()
      .slice(compiled.sql.toLowerCase().indexOf("order by"));
  ...
  it("select with sql template field", () => {
    ...
    const compiled = makeSelectBuilder({ cnt }).from(users).toSQL();
  ...
  it("limit() and offset() together", () => {
    const compiled = makeSelectBuilder().from(users).limit(10).offset(5).toSQL();
  ...
  it("offset() without limit() — valid SQL output", () => {
    const compiled = makeSelectBuilder().from(users).offset(20).toSQL();
    expect(compiled.sql.toLowerCase()).toContain("offset");
...
describe("INSERT edge cases", () => {
  ...
  it("throws when values() is called with empty array", () => {
    ...
    expect(() => builder.values([])).toThrow(/at least one value/);
  });

  it("insert subset of columns — omitted columns default, only provided columns produce params", () => {
    const compiled = makeInsertBuilder().values({ id: "u1", name: "Alice" }).toSQL();
...
describe("UPDATE edge cases", () => {
  ...
  it("update can set a column to null", () => {
    const compiled = makeUpdateBuilder().set({ name: null }).where(eq(users.id, "u1")).toSQL();
    expect(compiled.sql).toContain("`name` = ?");
