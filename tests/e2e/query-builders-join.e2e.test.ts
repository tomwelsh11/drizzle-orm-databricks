...
describe.skipIf(!hasCredentials())("Query builder JOINs (e2e)", () => {
  ...
  it("RIGHT JOIN includes posts without matching users", async () => {
    const db = getDb();
    const rows = await db.select().from(users).rightJoin(posts, eq(users.id, posts.userId));
  ...
  it("FULL JOIN returns all rows from both sides", async () => {
    const db = getDb();
    const rows = await db.select().from(users).fullJoin(posts, eq(users.id, posts.userId));
