...
function insertUsers(
  tbl: string,
  ...rows: Array<[string, string, number, number, boolean, number, string, string]>
) {
  const values = rows
    .map(
...
describe.skipIf(!hasCredentials())("Drizzle constructs (e2e)", () => {
  ...
  it("uses table reference in FROM clause", async () => {
    const db = getDb();
    const rows = await db.execute<Record<string, unknown>>(sql`SELECT * FROM ${users}`);
  ...
  it("uses column references in DELETE WHERE", async () => {
    ...
    await db.execute(sql`DELETE FROM ${users} WHERE ${users.id} = ${"u_del"}`);
    const rows = await db.execute<Record<string, unknown>>(
