  ...
  describe('INSERT', () => {
    it('inserts a single row and reads it back', async () => {
      ...
      const rows = await db
        .select()
        .from(items)
        .where(eq(items.id, 'insert-single'));
    ...
    it('round-trips all column types', async () => {
      ...
      const rows = await db
        .select()
        .from(items)
        .where(eq(items.id, 'insert-types'));
    ...
    it('inserts an empty string value', async () => {
      ...
      const rows = await db
        .select()
        .from(items)
        .where(eq(items.id, 'insert-empty'));
    ...
    it('inserts values containing special characters', async () => {
      ...
      const rows = await db
        .select()
        .from(items)
        .where(eq(items.id, 'insert-special'));
  ...
  describe('UPDATE', () => {
    it('updates a single column with WHERE and leaves others unchanged', async () => {
      ...
      const rows = await db
        .select()
        .from(items)
        .where(eq(items.id, 'update-single'));
    ...
    it('updates all rows when WHERE is omitted', async () => {
      ...
      await db.insert(items).values(
        ids.map((id) => ({ id, name: 'pre', value: 1, active: true, score: 0.0 })),
      );
    ...
    it('does not error when WHERE matches no rows', async () => {
      ...
      const rows = await db
        .select()
        .from(items)
        .where(eq(items.id, 'update-no-match-xyz'));
      expect(rows).toEqual([]);
    });
    ...
    it('updates a column to NULL', async () => {
      ...
      await db
        .update(items)
        .set({ name: null })
        .where(eq(items.id, 'update-to-null'));
  ...
  describe('DELETE', () => {
    it('deletes a specific row and leaves others intact', async () => {
      ...
      const target = await db
        .select()
        .from(items)
        .where(eq(items.id, 'delete-target'));
      const keep = await db
        .select()
        .from(items)
        .where(eq(items.id, 'delete-keep'));
    ...
    it('deletes with complex WHERE using and()/gt()', async () => {
      ...
      await db.delete(items).where(
        and(gt(items.value, 50), eq(items.active, true)),
      );
  ...
  describe('lifecycle', () => {
    it('insert then immediately select returns the inserted row', async () => {
      ...
      const rows = await db
        .select()
        .from(items)
        .where(eq(items.id, 'lifecycle-read'));
    ...
    it('update then select reflects the change', async () => {
      ...
      await db
        .update(items)
        .set({ name: 'after' })
        .where(eq(items.id, 'lifecycle-update'));
