...
describe.skipIf(!hasCredentials())('Query builder SELECT (e2e)', () => {
  ...
  it('WHERE gt / lt / gte / lte filter by comparison', async () => {
    const db = getDb();
    const gtRows = await db.select({ id: users.id, age: users.age }).from(users).where(gt(users.age, 45));
    expect(gtRows.every((r) => r.age > 45)).toBe(true);
    expect(gtRows.length).toBeGreaterThanOrEqual(2);

    const ltRows = await db.select({ id: users.id, age: users.age }).from(users).where(lt(users.age, 25));
    expect(ltRows.every((r) => r.age < 25)).toBe(true);
    expect(ltRows.length).toBeGreaterThanOrEqual(2);

    const gteRows = await db.select({ id: users.id, age: users.age }).from(users).where(gte(users.age, 45));
    expect(gteRows.every((r) => r.age >= 45)).toBe(true);
    expect(gteRows.length).toBeGreaterThanOrEqual(3);

    const lteRows = await db.select({ id: users.id, age: users.age }).from(users).where(lte(users.age, 25));
  ...
  it('ORDER BY ascending sorts rows in ascending order', async () => {
    const db = getDb();
    const rows = await db.select({ id: users.id, age: users.age }).from(users).orderBy(asc(users.age));
    expect(rows).toHaveLength(10);
  ...
  it('ORDER BY descending sorts rows in descending order', async () => {
    const db = getDb();
    const rows = await db.select({ id: users.id, age: users.age }).from(users).orderBy(desc(users.age));
    expect(rows).toHaveLength(10);
    for (let i = 1; i < rows.length; i++) {
  ...
  it('OFFSET with LIMIT paginates results', async () => {
    const db = getDb();
    const rows = await db.select({ id: users.id }).from(users).orderBy(asc(users.id)).limit(3).offset(3);
    expect(rows).toHaveLength(3);
